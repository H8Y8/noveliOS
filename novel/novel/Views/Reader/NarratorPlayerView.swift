...
enum SleepTimerOption: String, CaseIterable, Identifiable {
    ...
    var displayName: String {
        ...
        case .min15: String(localized: "15分")
        case .min30: String(localized: "30分")
        case .min45: String(localized: "45分")
        case .min60: String(localized: "60分")
        }
    }
...
struct NarratorPlayerView: View {
        ...
        sectionCard {
            ...
            HStack(spacing: NNSpacing.sm) {
                ...
                .accessibilityAdjustableAction { direction in
                    switch direction {
                    case .increment:
                        let newRate: Float = min(settings.ttsRate + 0.025, 0.7)
                        settings.ttsRate = newRate
                        ttsService.setRate(newRate)
                    case .decrement:
                        let newRate: Float = max(settings.ttsRate - 0.025, 0.3)
                        settings.ttsRate = newRate
                        ttsService.setRate(newRate)
                    @unknown default:
                        break
    ...
    private var speedLabel: String {
        ...
        case ..<0.38: String(localized: "慢速")
        case ..<0.46: String(localized: "偏慢")
        case ..<0.55: String(localized: "適中")
        case ..<0.63: String(localized: "偏快")
        default:       String(localized: "快速")
        }
    }
    ...
    private var synthesisStatusLabel: String {
        if synthesisService.isSynthesizing  { return String(localized: "合成中…") }
        if synthesisService.isComplete      { return String(localized: "已完成") }
        if synthesisService.synthesizedCount > 0 { return String(localized: "部分完成") }
        return String(localized: "尚未開始")
    }
    ...
    private var sleepTimerControl: some View {
        sectionCard {
            ...
            .sensoryFeedback(.selection, trigger: selectedSleepTimer)
        }
    }
