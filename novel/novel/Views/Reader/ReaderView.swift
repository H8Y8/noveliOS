...
struct ReaderView: View {
    ...
    // 背景快取建立任務（用於取消）
    @State private var buildCacheTask: Task<Void, Never>?
    ...
    private var currentChapter: Chapter? {
        guard currentChapterIndex >= 0, currentChapterIndex < cachedSortedChapters.count else { return nil }
        return cachedSortedChapters[currentChapterIndex]
    }
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            if cachedAllParagraphs.isEmpty {
                ...
                ScrollReaderView(
                    ...
                    fontFamily: settings.fontFamily,
                    highlightedParagraphIndex: ttsService.isPlaying ? ttsService.currentParagraphIndex : nil,
                    bookmarkedIndices: Set(book.bookmarks.map(\.paragraphIndex)),
                    onToggleBookmark: { toggleBookmark(at: $0) },
                    visibleParagraphIndex: $visibleParagraphIndex,
                    scrollCommand: $scrollCommand
                )
                .onTapGesture {
        ...
        .onDisappear {
            ...
            do {
                ...
                #if DEBUG
                print("⚠️ 閱讀進度儲存失敗：\(error.localizedDescription)")
                #endif
            }
            ttsService.stop()
        ...
        .sheet(isPresented: $showBookmarks) {
            BookmarkListSheet(book: book) { paragraphIndex in
                visibleParagraphIndex = paragraphIndex
                scrollCommand = ScrollCommand(index: paragraphIndex)
            }
        }
        .sheet(isPresented: $showSearch) {
            SearchSheet(paragraphs: cachedAllParagraphs) { paragraphIndex in
                visibleParagraphIndex = paragraphIndex
                scrollCommand = ScrollCommand(index: paragraphIndex)
            }
        }
    ...
    private var bottomToolbar: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: NNSpacing.xs) {
                // 章節進度滑桿（有多章節時才顯示）
                if cachedSortedChapters.count > 1 {
                    chapterSlider
                }
        ...
        VStack(spacing: 3) {
            ...
            HStack(spacing: NNSpacing.xs) {
                ...
                Slider(
                    ...
                    in: 0...Double(max(cachedSortedChapters.count - 1, 1)),
                    step: 1
                )
                .tint(theme.textColor.opacity(0.45))
                .accessibilityLabel("章節進度")
                .accessibilityValue("第 \(currentChapterIndex + 1) 章，共 \(cachedSortedChapters.count) 章")
                .sensoryFeedback(.impact(flexibility: .soft), trigger: currentChapterIndex)
                .accessibilityAdjustableAction { direction in
                    switch direction {
                    case .increment:
                        if currentChapterIndex < cachedSortedChapters.count - 1 {
                ...
                Text("\(cachedSortedChapters.count)")
                    .font(NNFont.uiCaption2)
                    .foregroundStyle(theme.secondaryTextColor)
                    .monospacedDigit()
    ...
    /// 切換目前可見段落的書籤狀態
    func toggleBookmark(at paragraphIndex: Int) {
        if let existing = book.bookmarks.first(where: { $0.paragraphIndex == paragraphIndex }) {
            modelContext.delete(existing)
        } else {
            guard paragraphIndex < cachedAllParagraphs.count else { return }
            let text = cachedAllParagraphs[paragraphIndex]
            let preview = String(text.prefix(50))
            let bookmark = Bookmark(paragraphIndex: paragraphIndex, preview: preview)
            bookmark.book = book
