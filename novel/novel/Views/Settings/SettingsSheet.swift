...
struct SettingsSheet: View {
    ...
    private var readingModeSection: some View {
        sectionCard(title: "閱讀模式", icon: "book.pages") {
            ...
            .sensoryFeedback(.selection, trigger: settings.pageMode)
        }
    }
        ...
        sectionCard(title: "字型", icon: NNSymbol.font) {
            ...
            VStack(spacing: NNSpacing.xs) {
                ...
                HStack(spacing: NNSpacing.sm) {
                    ...
                    .tint(NNColor.accent)
                    .accessibilityLabel("字型大小")
                    .accessibilityValue("\(Int(settings.fontSize)) pt")
                    .accessibilityAdjustableAction { direction in
                        switch direction {
                        case .increment:
                            settings.fontSize = min(settings.fontSize + 1, 32)
                        case .decrement:
                            settings.fontSize = max(settings.fontSize - 1, 14)
                        @unknown default:
                            break
                        }
                    }
    ...
    private var layoutSection: some View {
        sectionCard(title: "排版", icon: NNSymbol.lineHeight) {
            ...
            .sensoryFeedback(.selection, trigger: settings.lineSpacing)
        }
    }
    ...
    private var ttsEngineSection: some View {
        sectionCard(title: "語音朗讀", icon: NNSymbol.speakerWave) {
            ...
            .sensoryFeedback(.selection, trigger: settings.ttsProvider)
    ...
    private var edgeTTSSettings: some View {
        VStack(alignment: .leading, spacing: NNSpacing.sm) {
            ...
            HStack(spacing: NNSpacing.sm) {
                TextField("http://host:5050, http://host:5051", text: Binding(
        ...
        VStack(spacing: NNSpacing.xs) {
            ...
            HStack(spacing: NNSpacing.sm) {
                ...
                .accessibilityAdjustableAction { direction in
                    switch direction {
                    case .increment:
                        let newRate = min(settings.ttsRate + 0.05, 0.7)
                        settings.ttsRate = newRate
                        ttsService.setRate(newRate)
                    case .decrement:
                        let newRate = max(settings.ttsRate - 0.05, 0.3)
                        settings.ttsRate = newRate
                        ttsService.setRate(newRate)
                    @unknown default:
                        break
    ...
    private var synthesisHintText: String {
        if synthesisService.isSynthesizing {
            return String(localized: "\(synthesisService.synthesizedCount) / \(synthesisService.totalCount) 段，完成後關閉 app 不會遺失進度")
        } else if synthesisService.isComplete {
            return String(localized: "全書已合成完成，播放時完全不需等待。")
        } else if synthesisService.synthesizedCount > 0 {
            return String(localized: "進度已儲存，可隨時繼續合成剩餘部分。")
        } else {
            return String(localized: "預先合成全書語音，播放時無需等待網路。")
        }
    }
    ...
    private var rateDescription: String {
        if settings.ttsRate < 0.4 {
            return String(localized: "較慢")
        } else if settings.ttsRate < 0.55 {
            return String(localized: "適中")
        } else {
            return String(localized: "較快")
        }
    }
    ...
    private func checkServerStatus() {
        ...
        // 先同步 URL 到 provider，確保檢查的是最新設定的伺服器
        ttsService.setEdgeServerURL(settings.edgeTTSServerURL)
        Task {
...
private enum ServerStatus {
    ...
    var description: String {
        switch self {
        case .unknown: String(localized: "尚未測試")
        case .checking: String(localized: "連線中...")
        case .connected: String(localized: "已連線")
        case .disconnected: String(localized: "無法連線")
        }
    }
