...
final class EdgeTTSProvider: TTSProvider {
    ...
    /// Edge TTS server 的基礎 URL 列表，如 [http://192.168.1.100:5050, http://192.168.1.101:5050]
    var serverURLs: [URL] = []
    ...
    private var nextServerURL: URL? {
        guard !serverURLs.isEmpty else { return nil }
        counterLock.lock()
        defer { counterLock.unlock() }
        let index = _requestCounter % serverURLs.count
        _requestCounter += 1
    ...
    func synthesize(text: String, voice: TTSVoice, rate: Float) async throws -> Data {
        guard let baseURL = nextServerURL else {
    ...
    func isAvailable() async -> Bool {
        guard let baseURL = serverURLs.first else { return false }
